...
from cvep_processor import CvepProcessor
...
def draw_frame(gl_state, val, chosen):
  vbo, program, valsId, chosenId, ibo = gl_state
...
def cvep_loop(is_sync_frame, alive, chosen_val, record):
  emotiv_loop(CvepProcessor(), (is_sync_frame, alive, chosen_val), record)
...
def loop(record=True):
  ...
  window = init_window()
  program, vertex, fragment = init_shaders()
  vao, vbo, ibo = init_buffers()
  ...
  emotiv_process = multiprocessing.Process(
      target=cvep_loop,
      args=(is_sync_frame, alive, chosen_val, record))
